...
export default async function AdminOrderDetail({
  ...
  return (
    <section className="site-wrap py-12">
      <Link href="/admin/orders" className="text-sm text-sage-deep underline">
        ← Back to orders
      </Link>
      ...
      <div className="mt-3 flex flex-wrap items-start justify-between gap-4">
        ...
        {order.receipt_url && (
          <a href={order.receipt_url as string} target="_blank" rel="noreferrer" className="btn-ghost">
            Download Receipt
          </a>
        )}
      </div>
        ...
        <aside className="h-fit space-y-4">
          <div className="rounded-xl2 border border-line bg-white p-6">
            ...
            <form action={updateOrderStatus} className="mt-3 space-y-3">
              ...
              <select
                ...
                className="w-full rounded-xl border border-line bg-cream px-3 py-2.5 capitalize outline-none focus:border-sage-deep"
              >
          ...
          <div className="rounded-xl2 border border-line bg-white p-6">
            ...
            <p className="mt-2 text-xs text-ink-soft">
              {order.receipt_url
                ? "Receipt PDF stored. Use the button above to download."
                : "No receipt PDF yet — click below to generate."}
            </p>
            <form action={regenerateAction} className="mt-3">
              <input type="hidden" name="id" value={order.id as string} />
              <button className="btn-ghost w-full text-center">
                {order.receipt_url ? "Regenerate Receipt" : "Generate Receipt"}
              </button>
            </form>
          </div>
        </aside>
